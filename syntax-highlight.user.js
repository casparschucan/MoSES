...
(function () {
  ...
  // Tokenising and field classification live in the @require'd library, so
  // that this script and stack-lint.user.js cannot disagree about where a
  // string ends or which field is Maxima. Destructured here so every call
  // site below reads the same as when they were local functions.
  if (typeof MosesStackLang === 'undefined') {
    console.error(
      LOG_PREFIX + ' the shared library did not load (@require ' +
        'lib/stack-lang.v1.js), so highlighting is off. In Tampermonkey, ' +
        'check the script\'s Externals tab and re-install it if the entry ' +
        'is missing.'
    );
    return;
  }
